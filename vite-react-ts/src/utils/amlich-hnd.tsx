...
export function jdn2date(jd: number): [number, number, number] {
  let Z: number, A: number, alpha: number;
...
export function getYearInfo(yyyy: number) {
	let yearCode;
	if (yyyy < 1900) {
...
function getDayName(lunarDate:LunarDate) {
	...
	const s = "Ng\u00E0y " + cc[0] +", tháng "+cc[1] + ", n\u0103m " + cc[2];
	return s;
}
...
export function getCanChi(lunar: LunarDate) {
...
function getDayString(lunar: LunarDate, solarDay: string | number, solarMonth: string | number, solarYear: string | number) {
	let s;
...
export function getGioHoangDao(jd: number) {
...
class OutputOptions {
  fontSize: string = "13pt";
  tableWidth: string = "420px";
}

const PRINT_OPTS = new OutputOptions();

const FONT_SIZES = ["9pt", "13pt", "17pt"] as const;
const TAB_WIDTHS = ["180px", "420px", "600px"] as const;

export function setOutputSize(size: "small" | "normal" | "big") {
  let idx = 1;

  if (size === "small") idx = 0;
  else if (size === "big") idx = 2;

  PRINT_OPTS.fontSize = FONT_SIZES[idx];
  PRINT_OPTS.tableWidth = TAB_WIDTHS[idx];
}

export function printSelectedMonth() {
	getSelectedMonth();
...
export function printSelectedYear() {
	getSelectedMonth();
	return printYear(currentYear);
}

function printStyle() {
	const fontSize = PRINT_OPTS.fontSize;
	let res = "";
...
export function printTable(mm: number, yy: number): string | undefined {
...
function getPrevYearLink(mm: number, yy: number) {
	//return '<a href="'+window.location.pathname+'?yy='+(yy-1)+'&mm='+mm+'"><img src="left2.gif" width=16 height=12 alt="PrevYear" border=0></a>';
	return '<a href="'+window.location.pathname+'?yy='+(yy-1)+'&mm='+mm+'">&lt;&lt;</a>';
}

function getNextYearLink(mm: number, yy: number) {
...
function printCell(lunarDate: LunarDate, solarDate: string | number, solarMonth: string | number, solarYear: string | number) {
	...
	let lunar = String(lunarDate.day);
...
// function infoCellSelect(id: number) {
// 	if (id == 0) { /* empty */ }
// }

// function alertDayInfo(dd: number, mm: number, yy: number, leap: number, jd: number, sday: string, smonth: string, syear: string) {
...
// function showVietCal(): void {
//   const text = `${getCurrentTime()} -+- ${getTodayString()}`;

//   // ✅ Safe replacement for window.status
//   document.title = text;

//   // ✅ Proper recursive timeout
 ...
 */
export function convDLtoAL(yyyy: number, mm: number, dd: number) {
	const ld = getLunarDate(dd, mm, yyyy);
 ...
 * return: [yyyy, mm, dd]
 */
export function convALtoDL(
  yyyy: number,
  mm: number,
  dd: number,
  leap: number
): [number, number, number] {
  const yearInfo = getYearInfo(yyyy);

  const ld = yearInfo.find(
    item => item.month === mm && item.leap === leap
  );

  if (!ld) {
    throw new Error(
      `convALtoDL: invalid lunar date yyyy=${yyyy}, mm=${mm}, leap=${leap}`
    );
  }

  const [dd2, mm2, yyyy2] = jdn2date(ld.jd + dd - 1);
