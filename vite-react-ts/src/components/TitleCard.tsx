/* eslint-disable */
// @ts-nocheck
import { useState, useEffect, memo, useCallback, useRef, useMemo, type ReactNode, type ChangeEvent, type KeyboardEvent } from "react";
import type { SelectChangeEvent, IconButtonProps } from "@mui/material";
import {
  Alert,
  Checkbox,
  Chip,
...
function EditMenu({ onEdit, onDel, onCopy }: EditMenuProps) {
...
function EditTitleModal({ open, onClose, data, onSubmit }: { open: boolean; onClose: () => void; data: any; onSubmit: AnyFn; }) {
  const isMobile = useMediaQuery("(max-width:600px)");
  // const [editingTitle, setEditingTitle] = useState(data);
  const handleSave = ({ changes }: { changes: any }) => {
    onSubmit({ changes });
    onClose();
  };
...
function SortableItem({ id, children, isMobile, p }: SortableItemProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
 ...
 * return: (Date)
 **/
function dateFromString(text: string) {
...
export function TitleEditor({ name, isMobile, data, onSave, onClose, ctrlBar = false }: TitleEditorProps) {
  ...
  const [showLogModal, setShowLog] = useState<boolean>(false);
  ...
  const handleTagsChange = useCallback((tags: any) => {
    setLocalData((prev: any) => {
      const newData = { ...prev, tags };
      updateHis(newData);
      return newData;
    });
  }, []);
  const handleParagraphChange = useCallback((index: number, value: any) => {
    setLocalData((prev: any) => {
  ...
  const removeParagraph = useCallback((index: number) => {
    setLocalData((prev: any) => {
      const paragraphs = prev.paragraphs.filter((_: any, i: number) => i !== index);
      const newData = { ...prev, paragraphs };
      updateHis(newData);
      return newData;
    });
  ...
  const insertParagraph = useCallback((idx: number) => {
    setLocalData((prev: any) => {
      const paragraphs = [...prev.paragraphs];
  ...
  const moveParagraph = useCallback((fromIndex: number, toIndex: number) => {
    setLocalData((prev: any) => {
      const paragraphs = prev.paragraphs;
  ...
  const combineParagraph = useCallback((index: number) => {
    setLocalData((prev: any) => {
      const paragraphs = prev.paragraphs;
...
const ParagraphEditor = memo(function ParagraphEditor({
  ...
  return (
    <Box prosition="relative" sx={{ mb: 1 }}>
      <TextField
        sx={{
          // mt: 1,
          "& .MuiInputBase-input": {
